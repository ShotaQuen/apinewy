...
app.get('/api/shorturl', async (req, res) => {
  ...
  try {
    const { tinyurl } = require('./scrape')
    const response = await tinyurl(q);    res.status(200).json({
      status: true,
      creator: 'ikann',
      data: response
    });
...
app.get('/api/komiku', async (req, res) => {
  const { q } = req.query;
  ...
  try {
    const { komiku } = require('./scrape')
    const response = await komiku.search(q);
    res.status(200).json({
      status: true,
      creator: 'ikann',
      data: response
